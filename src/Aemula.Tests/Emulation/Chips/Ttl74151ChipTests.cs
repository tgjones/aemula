...
public class Ttl74151ChipTests
{
    [Test]
    ...
    {
        var chip = new Ttl74151Chip { A = a, B = b, C = c, S = false };
        ...
        await Assert.That(chip.W).IsEqualTo(false);
    }

    [Test]
    public async Task YIsLowWhenSelectedInputIsLow()
    {
        var chip = new Ttl74151Chip { A = false, B = false, C = false, S = false, D0 = false };

        await Assert.That(chip.Y).IsEqualTo(false);
        await Assert.That(chip.W).IsEqualTo(true);
    }

    [Test]
    public async Task OutputForcedLowWhenStrobeIsHigh()
    {
        var chip = new Ttl74151Chip { A = false, B = false, C = false, S = true, D0 = true };
