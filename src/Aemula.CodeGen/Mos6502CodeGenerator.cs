...
public class Mos6502CodeGenerator : IIncrementalGenerator
{
    ...
    private sealed class InstructionCodeBuilder
    {
        ...
        private void EncodeBranch(string register, bool value)
        {
            ...
            Add("_address = (ushort)((PC & 0xFF00) | (_ad & 0xFF));",
                "if ((_ad & 0xFF00) == (PC & 0xFF00))",
                "{",
                "    PC = _ad;",
                "    _irqCounter >>= 1;",
                "    // Rewinding the NMI shift register must not shift the bottom bit out of",
                "    // existence: an NMI edge latched in this very cycle is delayed by the",
                "    // early poll, not cancelled - the edge stays latched until serviced.",
                "    _nmiCounter = (ushort)((_nmiCounter >> 1) | (_nmiCounter & 1));",
                "    FetchNextInstruction();",
                "}",
                "else",
                "{",
