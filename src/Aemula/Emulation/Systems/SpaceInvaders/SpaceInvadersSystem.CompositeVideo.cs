...
// Television. See the plan's "Fidelity approach" section for why the latter
// two are C# rather than chip classes - short version: this board's real
...
public sealed partial class SpaceInvadersSystem
{
    ...
    // session as a 74LS55 - see the plan's chip inventory table). Wired the
    ...
    // own 6B "COMP BLANKING" net only ever reads as one flat level (the
    // plan's own open risk notes this - "whether VSYNC is serrated... wasn't
    // legible"), and the two equal-weight 1K legs found at the B8 summing
    ...
    //
    // So, matching the plan's explicit fallback ("if it can't be traced,
    // calibrate against known-good sync/black/white levels the same way
    // Apple II did") and following Atari2600System.CompositeVideo.cs's own
    ...
    // NTSC's per-line serration - explicitly accepted by the plan's own open
    // risks section ("An unserrated 4-line VSYNC would cause Television's
    // horizontal raster oscillator to lose lock for ~3-4 lines once per
    // field... cosmetic"). 3 lines, matching that estimate.
    ...
    // see the plan's Fidelity approach section. 13125/18304 is the exact
